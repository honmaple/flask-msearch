* flask-msearch
  [[https://pypi.python.org/pypi/Flask-Msearch][https://img.shields.io/badge/pypi-v0.2.9-brightgreen.svg]]
...
** Geo position
    To use geo position it's necessary create a string field and store a geohash in it.
   ...
   #+BEGIN_SRC python
    # models.py
     ...
     def w_search():
         keyword = request.args.get('keyword')
         results = Post.query.msearch(keyword,fields=['title'],limit=20, geofield='geo', geohash='<geohash>', geodistance='10km').filter(...)
         # or
         results = Post.query.filter(...).msearch(keyword,fields=['title'],limit=20).filter(...)
         # elasticsearch
   #+END_SRC
